...
void MapDrawer::SaveMapPoints(std::string filename) {
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend;i++)
    {
        ...
        verts.push_back(pos.at<float>(0));
        verts.push_back(pos.at<float>(1));
        verts.push_back(pos.at<float>(2));
...
void MapDrawer::SaveMapAsPly(std::string filename, std::vector<float> verts, std::vector<uint8_t> colors) {
    std::filebuf fb;
        ...
        fb.close();
        std::cout << "wrote map to: " << filename  << std::endl;
}
